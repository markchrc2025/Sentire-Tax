// form-1701Q.jsx — Quarterly Income Tax Return (Individuals, Estates & Trusts)
// Exports: Form1701Q

function Form1701Q({ tp, data, set, comp }) {
  ...
  const spouseOn = data.spouseIncome === "yes";

  // computation line A/B
  function CRow({ no, label, sub, base, roA, roB, valA, valB, strong }) {
    return (
  ...
  const A = comp.A, Bb = comp.B;

  return (
    <>
      {/* PAGE 1 */}
      <div className="bir-sheet">
        <BirHeader code="1701Q" date="January 2018 (ENCS)" page="1" title="Quarterly Income Tax Return" sub="For Individuals, Estates and Trusts" pcode="1701Q 01/18 ENCS P1" />
        <div className="bir-instr bb b" style={{ borderTop: 0 }}>Enter all required information in CAPITAL LETTERS using BLACK ink. Mark applicable boxes with an “X”. Two copies must be filed with the BIR and one held by the Tax Filer.</div>

        <div className="row b" style={{ borderTop: 0 }}>
          <div className="bir-cell inline br" style={{ width: 230 }}>
            <span className="lblgrp"><span className="bir-ino">1</span> <span className="bir-cap">For the Year</span></span>
            <div className="fld"><BirText field="year" data={data} set={set} placeholder="2024" /></div>
          </div>
          <div className="bir-cell inline br grow">
            <span className="lblgrp"><span className="bir-ino">2</span> <span className="bir-cap">Quarter</span></span>
          ...
          <div className="bir-cell inline" style={{ width: 220 }}>
            <span className="lblgrp"><span className="bir-ino">3</span> <span className="bir-cap">Amended?</span></span>
        ...
        <div className="bir-cell inline b" style={{ borderTop: 0 }}>
          <span className="lblgrp"><span className="bir-ino">7</span> <span className="bir-cap">Taxpayer/Filer Type</span></span>
          <div className="fld" style={{ gap: 14 }}>
            {[["single", "Single Proprietor"], ["prof", "Professional"], ["estate", "Estate"], ["trust", "Trust"]].map(([v, l]) =>
        ...
        <div className="bir-cell inline b" style={{ borderTop: 0, alignItems: "flex-start" }}>
          <span className="lblgrp" style={{ paddingTop: 4 }}><span className="bir-ino">8</span> <span className="bir-cap">ATC</span></span>
          <div className="fld atc-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "6px 14px" }}>
            <BirCkRow on={is("atc", "II012")} onClick={() => pick("atc", "II012")}><b>II012</b>&nbsp;Business-Graduated</BirCkRow>
            <BirCkRow on={is("atc", "II014")} onClick={() => pick("atc", "II014")}><b>II014</b>&nbsp;Profession-Graduated</BirCkRow>
            <BirCkRow on={is("atc", "II013")} onClick={() => pick("atc", "II013")}><b>II013</b>&nbsp;Mixed-Graduated</BirCkRow>
            <BirCkRow on={is("atc", "II015")} onClick={() => pick("atc", "II015")}><b>II015</b>&nbsp;Business-8%</BirCkRow>
            <BirCkRow on={is("atc", "II017")} onClick={() => pick("atc", "II017")}><b>II017</b>&nbsp;Profession-8%</BirCkRow>
            <BirCkRow on={is("atc", "II016")} onClick={() => pick("atc", "II016")}><b>II016</b>&nbsp;Mixed-8%</BirCkRow>
          </div>
        </div>
        <div className="bir-cell inline b" style={{ borderTop: 0 }}>
          <span className="lblgrp"><span className="bir-ino">9</span> <span className="bir-cap">Taxpayer/Filer’s Name</span></span>
          <div className="fld"><BirVal value={name} /></div>
        </div>
        <div className="row b" style={{ borderTop: 0 }}>
          <div className="bir-cell inline br grow"><span className="lblgrp"><span className="bir-ino">10</span> <span className="bir-cap">Registered Address</span></span><div className="fld"><BirVal value={tp ? [tp.address, tp.city].filter(Boolean).join(", ") : ""} /></div></div>
          <div className="bir-cell inline" style={{ width: 150 }}><span className="lblgrp"><span className="bir-ino">10A</span> <span className="bir-cap">ZIP</span></span><div className="fld"><BirVal value={tp && tp.zip} /></div></div>
        </div>

        {/* 16 tax rate + method */}
        <div className="bir-cell inline b" style={{ borderTop: 0, alignItems: "flex-start" }}>
          <span className="lblgrp" style={{ paddingTop: 4 }}><span className="bir-ino">16</span> <span className="bir-cap">Tax Rate &amp; Method</span></span>
          <div className="fld col" style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-start" }}>
            <div className="row" style={{ gap: 16 }}>
              <BirCkRow on={is("rateA", "graduated")} onClick={() => pick("rateA", "graduated")}>Graduated rates per Tax Table</BirCkRow>
              <BirCkRow on={is("rateA", "eight")} onClick={() => pick("rateA", "eight")}>8% on gross sales/receipts</BirCkRow>
            </div>
            {data.rateA !== "eight" && (
              <div className="row" style={{ gap: 16 }}>
                <span style={{ fontSize: 9.5, fontWeight: 700 }}>Method:</span>
                <BirCkRow on={is("methodA", "itemized")} onClick={() => pick("methodA", "itemized")}>Itemized Deduction</BirCkRow>
                <BirCkRow on={is("methodA", "osd")} onClick={() => pick("methodA", "osd")}>Optional Standard Deduction (40%)</BirCkRow>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="row b" style={{ borderTop: 0, background: "var(--shade2)", fontWeight: 700, alignItems: "center", minHeight: 24 }}>
          ...
          <div className="desc" style={{ flex: 1, fontSize: 11 }}>Particulars</div>
        ...
        <div className="b" style={{ borderTop: 0 }}>
          ...
          <div className="bir-line bt" style={{ background: "var(--shade2)" }}>
            <div className="num">31</div><div className="desc" style={{ fontWeight: 700 }}>Aggregate Amount Payable/(Overpayment) (Sum of 30A & 30B)</div>
            <div className="amtcell bl br" style={{ width: 300 }}><BirAmt ro value={comp.aggregate} /></div>
          </div>
        </div>
      ...
      <div className="bir-sheet">
        <BirHeader code="1701Q" date="January 2018 (ENCS)" page="2" title="Quarterly Income Tax Return" sub="For Individuals, Estates and Trusts" pcode="1701Q 01/18 ENCS P2" />
        <div className="row b" style={{ borderTop: 0 }}>
        ...
        <div className="row b" style={{ borderTop: 0, background: "var(--shade)", fontWeight: 700, alignItems: "center", minHeight: 24 }}>
          <div className="grow" style={{ fontSize: 10.6, padding: "2px 8px" }}>Schedule I – Graduated IT Rates (with Itemized or OSD)</div>
          <div className="amtcell bl br bir-amthead">A) Taxpayer/Filer</div>
          <div className="amtcell br bir-amthead">B) Spouse</div>
        </div>
        <div className="b" style={{ borderTop: 0 }}>
          ...
          <CRow no="38" label="Less: Allowable Deductions (Itemized entry or OSD auto-40%)" base="deduct" roA={A.method === "osd"} roB={Bb.method === "osd"} valA={A.deductions} valB={Bb.deductions} />
        ...
        <div className="row b" style={{ borderTop: 0, background: "var(--shade)", fontWeight: 700, alignItems: "center", minHeight: 24 }}>
          <div className="grow" style={{ fontSize: 10.6, padding: "2px 8px" }}>Schedule II – 8% IT Rate</div>
          <div className="amtcell bl br"></div><div className="amtcell br"></div>
        </div>
        <div className="b" style={{ borderTop: 0 }}>
        ...
        <div className="bir-section b" style={{ borderTop: 0 }}>Schedule III – Tax Credits/Payments</div>
        <div className="b" style={{ borderTop: 0 }}>
        ...
        <div style={{ marginTop: 6 }}>
          <table className="bir-taxtable">
            <thead><tr><th>If Taxable Income is</th><th>Tax Due</th></tr></thead>
            <tbody>
          ...
          <div style={{ fontSize: 8.6, color: "#666", marginTop: 2 }}>Rates effective Jan 1, 2023 onward (TRAIN). For 2018–2022 quarters, the prior table applies.</div>
        </div>
      </div>
    </>
  );
