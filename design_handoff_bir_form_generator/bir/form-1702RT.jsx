// form-1702RT.jsx — Annual ITR for Corporations/Partnerships — Regular Rate
// Exports: Form1702RT

function Form1702RT({ tp, data, set, comp }) {
  ...
  const incDate = tp && tp.incorpDate ? (() => { const [y, m, dd] = tp.incorpDate.split("-"); return m + "/" + dd + "/" + y; })() : "";

  function L1({ no, label, sub, field, ro, value, strong, bg, dim }) {
    return (
      <div className="bir-line bt" style={bg ? { background: "var(--shade2)" } : null}>
        <div className="num">{no}</div>
        <div className="desc" style={{ fontWeight: strong ? 700 : 400, opacity: dim ? 0.5 : 1 }}>{label}{sub && <small> {sub}</small>}</div>
        <div className="amtcell bl br" style={{ width: 220 }}><BirAmt field={field} data={data} set={set} ro={ro} value={value} dim={dim} /></div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="bir-sheet">
        <BirHeader code="1702-RT" date="January 2018 (ENCS)" page="1" title="Annual Income Tax Return" sub="Corporation, Partnership & Other Non-Individual Taxpayer — Subject Only to REGULAR Income Tax Rate" pcode="1702-RT 01/18 ENCS P1" />
        ...
        <div className="row b" style={{ borderTop: 0 }}>
          <div className="bir-cell inline br" style={{ width: 340 }}><span className="lblgrp"><span className="bir-ino">10</span> <span className="bir-cap">Date of Incorporation/Organization</span></span><div className="fld"><BirVal value={incDate} /></div></div>
          <div className="bir-cell inline br" style={{ width: 250 }}><span className="lblgrp"><span className="bir-ino">11</span> <span className="bir-cap">Contact Number</span></span><div className="fld"><BirVal value={tp && tp.phone} lower /></div></div>
          <div className="bir-cell inline grow"><span className="lblgrp"><span className="bir-ino">12</span> <span className="bir-cap">Email</span></span><div className="fld"><BirVal value={tp && tp.email} lower /></div></div>
        </div>
        <div className="bir-cell inline b" style={{ borderTop: 0 }}>
          <span className="lblgrp"><span className="bir-ino">13</span> <span className="bir-cap">Method of Deductions</span></span>
          <div className="fld" style={{ gap: 16 }}>
            <BirCkRow on={is("method", "itemized")} onClick={() => pick("method", "itemized")}>Itemized Deductions [Sec. 34(A-J), NIRC]</BirCkRow>
        ...
        <div className="b" style={{ borderTop: 0 }}>
          ...
          <L1 no="21" label="TOTAL AMOUNT PAYABLE / (Overpayment) (Sum of Items 16 and 20)" ro value={comp.i21} strong bg />
        </div>
        ...
        <div className="bir-cell b" style={{ borderTop: 0 }}>
          ...
          <div className="row" style={{ gap: 20, marginTop: 3 }}>
            ...
            <BirCkRow on={is("over", "tcc")} onClick={() => pick("over", "tcc")}>To be issued a TCC</BirCkRow>
            <BirCkRow on={is("over", "carry")} onClick={() => pick("over", "carry")}>To be carried over as tax credit next year</BirCkRow>
          </div>
        </div>
        ...
        <div className="b" style={{ borderTop: 0 }}>
          ...
          <div className="row">
            <div className="bir-sign br grow" style={{ padding: "20px 6px 4px" }}>
              ...
              <div style={{ borderTop: "0.7px solid var(--ln)", marginTop: 3, paddingTop: 3 }}>Signature over Printed Name of President/Principal Officer/Authorized Representative</div>
            </div>
            <div className="bir-sign grow" style={{ padding: "20px 6px 4px" }}>
              <BirText field="treasurer" data={data} set={set} />
              <div style={{ borderTop: "0.7px solid var(--ln)", marginTop: 3, paddingTop: 3 }}>Signature over Printed Name of Treasurer/Assistant Treasurer</div>
            </div>
          </div>
        </div>
      ...
      <div className="bir-sheet" data-rate={comp.method}>
        <BirHeader code="1702-RT" date="January 2018 (ENCS)" page="2" title="Annual Income Tax Return" sub="Corporation, Partnership & Other Non-Individual — Regular Income Tax Rate" pcode="1702-RT 01/18 ENCS P2" />
        <div className="row b" style={{ borderTop: 0 }}>
        ...
        <div className="b" style={{ borderTop: 0 }}>
          ...
          <div className="bir-line bt"><div className="num"></div><div className="desc" style={{ fontStyle: "italic", fontWeight: 700 }}>Less: Deductions Allowable under Existing Law</div><div className="amtcell bl br" style={{ width: 220 }}></div></div>
          ...
          <L1 no="38" label="OR: Optional Standard Deduction (OSD) — 40% of Item 33" ro value={comp.i38} strong dim={comp.method !== "osd"} />
          ...
          <div className="bir-line bt"><div className="num"></div><div className="desc" style={{ fontStyle: "italic", fontWeight: 700 }}>Less: Tax Credits/Payments (attach proof)</div><div className="amtcell bl br" style={{ width: 220 }}></div></div>
        ...
        <div style={{ marginTop: 6, fontSize: 10, color: "#555" }}>
          Regular corporate rate is <b>25%</b> (or <b>20%</b> if net taxable income ≤ ₱5M and total assets ≤ ₱100M, excluding land). Tax due is the higher of the normal tax or the 2% MCIT.
        </div>
      </div>
    </>
  );
