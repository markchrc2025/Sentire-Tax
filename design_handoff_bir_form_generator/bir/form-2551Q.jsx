// form-2551Q.jsx — Quarterly Percentage Tax Return
// Exports: Form2551Q

function Form2551Q({ tp, data, set, comp }) {
  const pick = (f, v) => set(f, data[f] === v ? "" : v);
  const is = (f, v) => data[f] === v;
  const { fmtAmt } = window.BIR;
  ...
  return (
    <>
      ...
      <div className="bir-sheet">
        <BirHeader code="2551Q" date="January 2018 (ENCS)" page="1" title="Quarterly Percentage Tax Return" pcode="2551Q 01/18 P1" />
          ...
          <div className="bir-cell inline br grow">
            ...
            <div className="fld" style={{ gap: 10 }}>
              {["1st", "2nd", "3rd", "4th"].map((q) => (
                <BirCkRow key={q} on={is("quarter", q)} onClick={() => pick("quarter", q)}>{q}</BirCkRow>
              ))}
            </div>
          </div>
        </div>
        <div className="row b" style={{ borderTop: 0 }}>
          <div className="bir-cell inline br grow">
            <span className="lblgrp"><span className="bir-ino">4</span> <span className="bir-cap">Amended Return?</span></span>
          ...
          <div className="bir-cell inline" style={{ width: 290 }}>
            <span className="lblgrp"><span className="bir-ino">5</span> <span className="bir-cap">Number of Sheet/s Attached</span></span>
            <div className="fld" style={{ width: 90 }}><BirText field="sheets" data={data} set={set} /></div>
          </div>
        </div>

        <PartBand>Part I – Background Information</PartBand>
        <BgInfoReturn tp={tp} data={data} set={set} startNo={6} />

        <div className="bir-cell inline b" style={{ borderTop: 0 }}>
          <span className="lblgrp"><span className="bir-ino">12</span> <span className="bir-cap">Availing of tax relief under Special Law or Treaty?</span></span>
          <div className="fld" style={{ gap: 10 }}>
            ...
            <span style={{ fontSize: 9.5, marginLeft: 8 }}><b>12A</b> If yes, specify</span>
            <div style={{ width: 180 }}><BirText field="taxReliefSpec" data={data} set={set} /></div>
          </div>
        </div>

        <div className="bir-cell inline b" style={{ borderTop: 0 }}>
          <span className="lblgrp"><span className="bir-ino">13</span> <span className="bir-cap">Income tax rate availed (individuals under Sec. 116)</span></span>
          <div className="fld" style={{ gap: 16 }}>
            <BirCkRow on={is("itRate", "graduated")} onClick={() => pick("itRate", "graduated")}>Graduated income tax rate</BirCkRow>
            <BirCkRow on={is("itRate", "eight")} onClick={() => pick("itRate", "eight")}>8% income tax rate on gross sales/receipts</BirCkRow>
          </div>
        </div>
        ...
        <div className="b" style={{ borderTop: 0 }}>
          ...
          <div className="bir-line bt"><div className="num"></div><div className="desc" style={{ fontStyle: "italic", fontWeight: 700 }}>Less: Tax Credit/Payment (attach proof)</div><div className="amtcell bl br" style={{ width: 220 }}></div></div>
          ...
          <LineAmt no="24" label="TOTAL AMOUNT PAYABLE/(Overpayment) (Sum of Items 19 and 23)" ro value={comp.i24} data={data} set={set} strong bg />
        </div>

        <div className="bir-cell inline b" style={{ borderTop: 0 }}>
          <span className="lblgrp"><span className="bir-capi">If overpayment, mark one box only:</span></span>
          <div className="fld" style={{ gap: 20 }}>
            <BirCkRow on={is("over", "refund")} onClick={() => pick("over", "refund")}>To be refunded</BirCkRow>
            <BirCkRow on={is("over", "tcc")} onClick={() => pick("over", "tcc")}>To be issued a Tax Credit Certificate</BirCkRow>
          </div>
        </div>
      ...
      {/* PAGE 2 — Schedule 1 */}
      <div className="bir-sheet">
        <BirHeader code="2551Q" date="January 2018 (ENCS)" page="2" title="Quarterly Percentage Tax Return" pcode="2551Q 01/18 P2" />
        <div className="row b" style={{ borderTop: 0 }}>
        ...
        <div className="bir-section b" style={{ borderTop: 0 }}>Schedule 1 – Computation of Tax</div>
        <div className="row b" style={{ borderTop: 0, background: "var(--shade2)", fontWeight: 700, fontSize: 10 }}>
          <div style={{ width: 40, padding: "3px 4px", textAlign: "center" }} className="br">#</div>
          <div style={{ width: 240, padding: "3px 6px" }} className="br">Alphanumeric Tax Code (ATC)</div>
          <div className="grow br" style={{ padding: "3px 6px", textAlign: "right" }}>Taxable Amount</div>
          <div style={{ width: 90, padding: "3px 6px", textAlign: "center" }} className="br">Tax Rate</div>
          <div style={{ width: 170, padding: "3px 6px", textAlign: "right" }}>Tax Due</div>
        </div>
        {rows.map((r, i) => (
          <div className="row b" style={{ borderTop: 0 }} key={i}>
            <div style={{ width: 40, textAlign: "center", fontWeight: 700, fontSize: 11, paddingTop: 4 }} className="br">{i + 1}</div>
            <div style={{ width: 240 }} className="br">
              <select className="bir-select"
                value={r.atc || ""} onChange={(e) => {
                  const found = ATC.find((a) => a[0] === e.target.value);
                  setRow(i, "atc", e.target.value);
                  if (found) setRow(i, "rate", found[2]);
                }}>
                <option value="">— select —</option>
                {ATC.map((a) => <option key={a[0]} value={a[0]}>{a[0]} · {a[1].length > 48 ? a[1].slice(0, 48) + "…" : a[1]}</option>)}
              </select>
            </div>
            <div className="grow br"><BirAmt field={"taxable"} data={r} set={(k, v) => setRow(i, "taxable", v)} /></div>
            <div style={{ width: 90, display: "flex", alignItems: "center", justifyContent: "center", gap: 2 }} className="br">
              <input className="bir-rate" inputMode="decimal" value={r.rate == null ? "" : r.rate}
                onChange={(e) => setRow(i, "rate", e.target.value.replace(/[^0-9.]/g, ""))} />
              <span style={{ fontSize: 11, color: "#555" }}>%</span>
            </div>
            <div style={{ width: 170 }}><BirAmt ro value={comp.rows[i] ? comp.rows[i].due : 0} /></div>
          </div>
        ))}
        <div className="row b" style={{ borderTop: 0, background: "var(--shade2)" }}>
          <div className="grow" style={{ padding: "4px 6px", fontWeight: 700, fontSize: 11 }}>7 · Total Tax Due (Sum of Items 1 to 6) (To Part II Item 14)</div>
          <div style={{ width: 170 }} className="bl"><BirAmt ro value={comp.i14} /></div>
        </div>

        <div style={{ marginTop: 8 }}>
          <div style={{ fontSize: 10.6, fontWeight: 700, marginBottom: 3 }}>Table 1 – Alphanumeric Tax Code (ATC)</div>
          <table className="bir-taxtable">
            <thead><tr><th style={{ width: 70 }}>ATC</th><th>Percentage Tax On</th><th style={{ width: 60 }}>Rate</th></tr></thead>
            <tbody>{ATC.map((a) => <tr key={a[0]}><td style={{ fontWeight: 700 }}>{a[0]}</td><td>{a[1]}</td><td style={{ textAlign: "center" }}>{a[2]}%</td></tr>)}</tbody>
          </table>
        </div>
      </div>
    </>
