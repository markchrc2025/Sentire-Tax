// bir-guided-1701A.jsx — Google-Form-style, by-Part guided entry for 1701A
// Exports to window: Guided1701A
const { useState: gUseState } = React;

function Guided1701A({ tp, data, set, comp, onViewForm, onPrint }) {
  const { fmtAmt, num } = window.BIR;
  const [step, setStep] = gUseState(0);
  const pick = (f, v) => set(f, v);
  const is = (f, v) => data[f] === v;
  const name = tp ? (tp.kind === "individual" ? [tp.lastName, tp.firstName, tp.middleName].filter(Boolean).join(", ") : tp.regName) : "";
  ...
  const Money = ({ field }) => (
    <div className="g-money">
      ...
      <input inputMode="decimal" value={data[field] == null ? "" : data[field]} placeholder="0"
        ...
        onBlur={(e) => { const n = num(e.target.value); set(field, e.target.value.trim() === "" ? "" : String(window.BIR.roundPeso(n))); }} />
    </div>
  );
  const Pair = ({ base }) => spouseOn ? (
    <div className="g-pair">
      <div><div className="g-pair-h">Taxpayer / Filer</div><Money field={base + "A"} /></div>
      <div><div className="g-pair-h">Spouse</div><Money field={base + "B"} /></div>
    </div>
  ) : <Money field={base + "A"} />;
  ...
  /* ---- steps ---- */
  const atcOptions = rate === "eight"
    ? [{ val: "II015", code: "II015", title: "Business Income — 8% IT Rate" }, { val: "II017", code: "II017", title: "Income from Profession — 8% IT Rate" }]
    : [{ val: "II012", code: "II012", title: "Business Income — Graduated IT Rates" }, { val: "II014", code: "II014", title: "Income from Profession — Graduated IT Rates" }];
      ...
      <>
        <div className="g-readonly">
          ...
          <div className="g-ro-grid">
            <div className="g-ro-item"><i>Taxpayer</i><b>{name || "—"}</b></div>
            <div className="g-ro-item"><i>TIN</i><b>{tp ? tp.tin : "—"}</b></div>
            <div className="g-ro-item"><i>RDO Code</i><b>{tp ? tp.rdo : "—"}</b></div>
            <div className="g-ro-item"><i>Registered address</i><b>{tp ? [tp.address, tp.city, tp.zip].filter(Boolean).join(", ") : "—"}</b></div>
          </div>
        </div>
        ...
        <Q item="Item 2" label="Is this an Amended Return?" help="Choose Yes only if you are correcting a return you already filed."><YesNo field="amended" /></Q>
        <Q item="Item 3" label="Is this a Short Period Return?"><YesNo field="shortPeriod" /></Q>
        <Q item="Item 6" label="Taxpayer Type" req>
          <Cards field="taxpayerType" cols={2} options={[
        ...
        <Q item="Item 16" label="Civil Status">
          <Cards field="civil" cols={2} options={[
            { val: "single", title: "Single" }, { val: "married", title: "Married" },
            { val: "sep", title: "Legally Separated" }, { val: "widow", title: "Widow/er" },
          ]} />
        </Q>
        {married && (
          <Q item="Item 17" label="Does your spouse also earn income?" help="If yes, you’ll be able to enter the spouse’s figures alongside yours.">
  ...
  steps.push({
    part: "Part I", tab: "Tax rate", title: "Choose your tax rate",
    ...
    render: () => (
      <>
        <Q item="Item 19" label="Which tax rate are you using?" req>
          <Cards field="taxRate" options={[
            { val: "graduated", title: "Graduated rates with OSD", note: "Standard progressive tax (0%–35%). 40% Optional Standard Deduction is applied to your gross sales/receipts." },
            { val: "eight", title: "8% flat income tax rate", note: "8% on gross sales/receipts above ₱250,000, in lieu of graduated rates + percentage tax. Available only if gross sales do not exceed ₱3,000,000." },
          ]} />
        </Q>
        {rate && (
          <Q item="Item 7" label="Alphanumeric Tax Code (ATC)" help="Pick the code that matches your income source. We’ve narrowed it to the codes valid for your chosen rate." req>
  ...
  steps.push({
    part: "Part IV", tab: "Income", title: rate === "eight" ? "Income (8% rate)" : "Income & deductions",
    desc: rate === "eight"
    ...
    render: () => rate === "eight" ? (
      <>
        <Q item="Item 47" label="Sales / Revenues / Receipts / Fees" help="Total gross sales or receipts for the year (net of returns below)." req><Pair base="i47" /></Q>
        <Q item="Item 48" label="Less: Sales Returns, Allowances and Discounts"><Pair base="i48" /></Q>
        <Q item="Item 50–51" label="Other Non-Operating Income" help="Any other taxable income not from your main operations.">
          ...
          <div style={{ height: 8 }} /><Pair base="i50" />
        </Q>
      ...
      <>
        <Q item="Item 36" label="Sales / Revenues / Receipts / Fees" help="Total gross sales or receipts for the year." req><Pair base="i36" /></Q>
        <Q item="Item 37" label="Less: Sales Returns, Allowances and Discounts"><Pair base="i37" /></Q>
        <Q item="Item 41–42" label="Other Non-Operating Income" help="Other taxable income not from your main operations (optional).">
          ...
          <div style={{ height: 8 }} /><Pair base="i41" />
        </Q>
        <Q item="Item 43" label="Share in income from a General Professional Partnership (GPP)"><Pair base="i43" /></Q>
        <div className="g-result hl">
  ...
  steps.push({
    part: "Part IV.C", tab: "Credits", title: "Tax credits & payments",
    ...
    render: () => (
      <>
        ...
        <div className="g-result hl">
          ...
          <div className="g-result-row big"><span>Net tax payable (Item 65)</span><b className={comp.A.i65 < 0 ? "" : ""}>{P}{fmtAmt(comp.A.i65)}</b></div>
        </div>
      </>
    ),
  });
  ...
  steps.push({
    part: "Part II", tab: "Payable", title: "Total tax payable",
    ...
    render: () => (
      <>
        ...
        <div className="g-subsec">
          <div className="g-subsec-h"><Icon d={SIco.warn} size={15} style={{ color: "#d08a2e" }} />Penalties (only if filed late)</div>
        ...
        <div className="g-result hl">
          ...
          <div className="g-result-row big"><span>{overpay ? "Aggregate overpayment" : "Aggregate amount payable"} (Item 30)</span><b>{P}{fmtAmt(comp.i30)}</b></div>
        </div>
      </>
    ),
  });
  ...
  steps.push({
    part: "Part III", tab: "Payment", title: "Details of payment",
    desc: "How are you paying? Fill in only the rows that apply — you can skip this and add it later.",
    render: () => (
      <>
        {[["32", "Cash / Bank Debit Memo", "p32"], ["33", "Check", "p33"], ["34", "Tax Debit Memo", "p34"]].map(([no, lbl, k]) => (
          <Q key={k} item={"Item " + no} label={lbl}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, maxWidth: 420 }}>
              <Txt field={k + "bank"} ph="Drawee bank / agency" /><Txt field={k + "num"} ph="Number" />
              <Txt field={k + "date"} ph="Date (MM/DD/YYYY)" />
              <div className="g-money"><span className="peso">₱</span><input inputMode="decimal" value={data[k + "amt"] || ""} placeholder="0" onChange={(e) => set(k + "amt", e.target.value.replace(/[^0-9.\-]/g, ""))} /></div>
            </div>
          </Q>
        ))}
      </>
  ...
  // STEP — Review
  const checks = window.validate1701A ? window.validate1701A(data, comp) : [];
  steps.push({
    ...
    render: () => (
      <>
        <div className="g-result hl" style={{ marginTop: 0 }}>
          ...
          <div className="g-result-row"><span>Year · Rate</span><b>{data.year || "—"} · {rate === "eight" ? "8% flat" : rate === "graduated" ? "Graduated + OSD" : "—"}</b></div>
        ...
        <div className="g-subsec">
          ...
          {checks.length === 0 ? (
            <ul className="g-review-list"><li><Icon d={SIco.check} size={15} className="g-rev-ok" />Everything looks complete — ready to print &amp; file.</li></ul>
          ) : (
            <ul className="g-review-list">
              {checks.map((c, i) => <li key={i}><Icon d={SIco.warn} size={15} className="g-rev-warn" />{c.msg}</li>)}
            </ul>
          )}
        </div>
...
window.Guided1701A = Guided1701A;
