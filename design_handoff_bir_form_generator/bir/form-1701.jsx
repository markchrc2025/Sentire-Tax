// form-1701.jsx — Annual Income Tax Return (Individuals — mixed income / business+compensation)
// Exports: Form1701

function Form1701({ tp, data, set, comp }) {
  ...
  return (
    <>
      ...
      <div className="bir-sheet">
        <BirHeader code="1701" date="January 2018 (ENCS)" page="1" title="Annual Income Tax Return" sub="Individuals (incl. MIXED Income Earners), Estates and Trusts" pcode="1701 01/18 ENCS P1" />
        ...
        <div className="row b" style={{ borderTop: 0 }}>
          ...
          <div className="bir-cell inline" style={{ width: 230 }}>
            <span className="lblgrp"><span className="bir-ino">4</span> <span className="bir-cap">Short Period?</span></span>
            <div className="fld" style={{ gap: 12 }}>
        ...
        <div className="bir-cell inline b" style={{ borderTop: 0 }}>
          <span className="lblgrp"><span className="bir-ino">8</span> <span className="bir-cap">Taxpayer Type</span></span>
          <div className="fld" style={{ gap: 14 }}>
            {[["comp_biz", "Compensation & Business/Profession (Mixed)"], ["biz", "Business/Profession only"], ["estate", "Estate"], ["trust", "Trust"]].map(([v, l]) =>
              <BirCkRow key={v} on={is("taxpayerType", v)} onClick={() => pick("taxpayerType", v)}>{l}</BirCkRow>)}
          </div>
        </div>
        <div className="bir-cell inline b" style={{ borderTop: 0 }}>
          <span className="lblgrp"><span className="bir-ino">9</span> <span className="bir-cap">Taxpayer’s Name</span></span>
          <div className="fld"><BirVal value={name} /></div>
        </div>
        <div className="row b" style={{ borderTop: 0 }}>
          <div className="bir-cell inline br grow"><span className="lblgrp"><span className="bir-ino">11</span> <span className="bir-cap">Registered Address</span></span><div className="fld"><BirVal value={tp ? [tp.address, tp.city].filter(Boolean).join(", ") : ""} /></div></div>
          <div className="bir-cell inline" style={{ width: 150 }}><span className="lblgrp"><span className="bir-ino">11A</span> <span className="bir-cap">ZIP</span></span><div className="fld"><BirVal value={tp && tp.zip} /></div></div>
        </div>
        <div className="row b" style={{ borderTop: 0 }}>
          <div className="bir-cell inline br" style={{ width: 320 }}><span className="lblgrp"><span className="bir-ino">14</span> <span className="bir-cap">Citizenship</span></span><div className="fld"><BirVal value={tp && tp.citizenship} /></div></div>
          <div className="bir-cell inline grow"><span className="lblgrp"><span className="bir-ino">16</span> <span className="bir-cap">Email</span></span><div className="fld"><BirVal value={tp && tp.email} lower /></div></div>
        </div>

        {/* tax rate + method */}
        <div className="bir-cell inline b" style={{ borderTop: 0, alignItems: "flex-start" }}>
          <span className="lblgrp" style={{ paddingTop: 4 }}><span className="bir-ino">19</span> <span className="bir-cap">Tax Rate &amp; Method</span></span>
          <div className="fld" style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-start" }}>
            <div className="row" style={{ gap: 16 }}>
              <BirCkRow on={is("rateA", "graduated")} onClick={() => pick("rateA", "graduated")}>Graduated rates</BirCkRow>
              <BirCkRow on={is("rateA", "eight")} onClick={() => pick("rateA", "eight")}>8% on business gross sales/receipts (≤₱3M)</BirCkRow>
            </div>
            {data.rateA !== "eight" && (
              <div className="row" style={{ gap: 16 }}>
                ...
                <BirCkRow on={is("methodA", "osd")} onClick={() => pick("methodA", "osd")}>OSD (40%)</BirCkRow>
              </div>
            )}
          </div>
        </div>
        ...
        <PartBand>Part II – Total Tax Payable <span style={{ fontWeight: 400, fontStyle: "italic" }}>(DO NOT enter Centavos)</span></PartBand>
        ...
        <div className="b" style={{ borderTop: 0 }}>
          ...
          <div className="bir-line bt" style={{ background: "var(--shade2)" }}>
            <div className="num">30</div><div className="desc" style={{ fontWeight: 700 }}>Aggregate Amount Payable/(Overpayment) (Sum of 29A & 29B)</div>
      ...
      <div className="bir-sheet">
        ...
        <div className="row b" style={{ borderTop: 0, background: "var(--shade)", fontWeight: 700, alignItems: "center", minHeight: 24 }}>
          <div className="grow" style={{ fontSize: 10.6, padding: "2px 8px" }}>Section A – Compensation Income</div>
          <div className="amtcell bl br bir-amthead">A) Taxpayer/Filer</div>
          <div className="amtcell br bir-amthead">B) Spouse</div>
        </div>
        <div className="b" style={{ borderTop: 0 }}>
        ...
        <div className="row b" style={{ borderTop: 0, background: "var(--shade)", fontWeight: 700, alignItems: "center", minHeight: 24 }}>
          <div className="grow" style={{ fontSize: 10.6, padding: "2px 8px" }}>Section B – Business/Profession Income (Graduated: Itemized or OSD)</div>
        ...
        <div className="b" style={{ borderTop: 0 }}>
          ...
          <CRow no="" label="Less: Allowable Deductions (Itemized entry or OSD auto-40%)" base="deduct" roA={A.method === "osd"} roB={Bb.method === "osd"} valA={A.deductions} valB={Bb.deductions} />
        ...
        <div className="bir-section b" style={{ borderTop: 0 }}>Section C – Tax Due</div>
        <div className="b" style={{ borderTop: 0 }}>
        ...
        <div className="bir-section b" style={{ borderTop: 0 }}>Section D – Tax Credits/Payments</div>
        <div className="b" style={{ borderTop: 0 }}>
          <CRow no="" label="Prior Year’s Excess Credits" base="excess" valA={A.excess} valB={Bb.excess} />
        ...
        <div style={{ marginTop: 6 }}>
          <table className="bir-taxtable">
            <thead><tr><th>If Taxable Income is</th><th>Tax Due (2023 onward)</th></tr></thead>
            <tbody>
